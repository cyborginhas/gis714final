...
#3A. FIA data
aa.cs.ras <- rasterize(x=aa.cs.pts, y=myExpl_1k[[1]], fun='count', background=0);
aa.cs.ras <- (aa.cs.ras*((myExpl_1k[[1]]*0)+1))>0
...
sum(na.omit(myResp))
myBiomodData.cs1k <- BIOMOD_FormatingData(resp.var = myResp,
                                         expl.var = myExpl_1k,
                                         resp.xy = myRespXY,
...
plot(myBiomodData.cs1k)
saveRDS(myBiomodData.cs1k, file = "myBiomodData.cs.rds")#2752 presences

#3Ai. Defining Models Options using default options.
myBiomodOption <- BIOMOD_ModelingOptions()
#3Aii. Computing the models
getwd()#outputs placed here
myBiomodModelOut.cs1k <- biomod2::BIOMOD_Modeling(myBiomodData.cs1k,
                                                 ...
                                                 modeling.id=paste(myRespName,"FirstModeling.cs1k",sep=""))
saveRDS(myBiomodModelOut.cs1k, file = "myBiomodModelOut.cs1k.rds")
myBiomodModelEval.cs1k <- get_evaluations(myBiomodModelOut.cs1k) # get all models evaluation
dimnames(myBiomodModelEval.cs1k) # print the dimnames of this object
allmodels.cs1k.eval<-myBiomodModelEval.cs1k[c('TSS'),"Testing.data",,,] # print the eval scores of all selected models
allmodels.cs1k.eval<-as.data.frame(allmodels.cs1k.eval)
names(allmodels.cs1k.eval)<-"TSS"
models<-rownames(allmodels.cs1k.eval)
res<-1000
db<-"cs"
allmodels<-cbind(models,as.data.frame(allmodels.cs1k.eval),res,db)
write.table(allmodels,"allmodels.csv", sep=",", row.names = FALSE,col.names = TRUE,append=TRUE)

vars_importance.cs1k <- as.data.frame(get_variables_importance(myBiomodModelOut.cs1k)) # print variable 
var_means<-rowMeans((vars_importance.cs1k),na.rm=TRUE)
res<-1000
db<-"cs"
pred<-names(var_means)
var_means<-cbind(pred,as.data.frame(var_means),res,db)

#tiff('~/Desktop/tohexports/cs1kmeanvarimp.tiff', units="in", width=8, height=8, res=300, compression = 'lzw')
#barplot(colMeans(vars_imp_means_cs1k), ylab="Mean Variance Importance Score", xlab="Predictors")
#dev.off()
var_rank<-rank((vars_importance.cs1k)*-1,ties.method="average",na.last=NA)
...
myBiomodEM.cs1k <- BIOMOD_EnsembleModeling(modeling.output = myBiomodModelOut.cs1k,
                                      chosen.models = 'all',
                                      em.by='all',
                                      eval.metric = c('TSS'),
...
saveRDS(myBiomodEM.cs1k, file = "myBiomodEM.cs1k.rds")
ensmodeleval<-get_evaluations(myBiomodEM.cs1k) # get evaluation scores
...
myBiomodProj.cs1k <- BIOMOD_Projection(modeling.output = myBiomodModelOut.cs1k,
                                  new.env = myExpl_1k,
                                  ...
                                  clamping.mask = F,
                                  output.format = '.grd')
saveRDS(myBiomodProj.cs1k, file = "myBiomodProj.cs1k.rds")
myCurrentProj.cs <- get_predictions(myBiomodProj.cs1k) # if you want to make custom plots, you can also get the projected map
myBiomodEF.cs1k <- BIOMOD_EnsembleForecasting(EM.output = myBiomodEM.cs1k,
                                         projection.output = myBiomodProj.cs1k)
saveRDS(myBiomodEF.cs1k, file = "myBiomodEF.cs1k.rds")
plot(myBiomodEF.cs1k)
pred.out.cs1k <- myBiomodEF.cs1k@proj@val[[1]]
pred.out.cs1k[]<-pred.out.cs1k[]/1000
plot(pred.out.cs1k)
writeRaster(pred.out.cs1k, filename = 'toh_ens_cs1k_wgs.tif', format="GTiff")
crs<-"+proj=lcc +lat_1=20 +lat_2=60 +lat_0=40 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs" 
pred.out.cs1k.lcc<-(projectRaster(pred.out.cs1k,crs=crs))
writeRaster(pred.out.cs1k.lcc, filename = 'toh_ens_cs1k_lcc.tif', format="GTiff")

#3Av. output plot
rpts <- rasterToPoints(pred.out.cs1k.lcc)
